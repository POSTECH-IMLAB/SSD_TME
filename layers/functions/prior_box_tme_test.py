...
class PriorBoxTme_test(object):
    ...
    def __init__(self, cfg):
        super(PriorBoxTme_test, self).__init__()
        # self.type = cfg.name
        ...
        for k, (f_w, f_h) in enumerate(zip(self.feature_maps_w, self.feature_maps_h)):
            ...
            for i in range(f_h):
                for j in range(f_w):
                    ...
                    # aspect_ratio: 1
                    # rel size: min_size
                    s_k_w = self.min_sizes[k] / (self.image_size_w / 2)
                    s_k_h = self.min_sizes[k] / self.image_size_h
                    mean += [cx, cy, s_k_w, s_k_h]

                    # aspect_ratio: 1
                    # rel size: sqrt(s_k * s_(k+1))
                    s_k_prime_w = sqrt(s_k_w * (self.max_sizes[k] / (self.image_size_w / 2)))
